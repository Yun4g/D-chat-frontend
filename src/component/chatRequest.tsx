...
function ChatRequest() {
  ...
  const handleOpenChat = (friend: FriendsType) => {
    if (!friend.user) return; // safety check
    dispatch(openChat());
    dispatch(
      setSelectedUser({
        _id: friend.user._id,
  ...
  // Filter friends safely
  const filteredDisplay = friends.filter((friend) => {
    if (!friend.user) return false; // skip null users
        ...
        {!loading && !error && filteredDisplay.length > 0 && (
          <div className="px-3 space-y-2">
            {filteredDisplay.map((friend) => (
              <div
                key={friend.roomId} // using roomId as key
              ...
              >
                <div className="h-10 w-10 rounded-full overflow-hidden bg-gray-700">
                  <img
                    src={friend.user?.avatarUrl || "/default-avatar.png"} // fallback avatar
